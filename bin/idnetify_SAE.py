...
if options.wellington_footprints_switch == "on" and options.macs2_call_peak_switch == "on" :
    if not options.output_name or not options.output_path or not options.input_file or not options.input_bam_file or not options.chrom_size_file:
        ...
        parser.print_help()
        exit()
elif options.wellington_footprints_switch == "on" and options.macs2_call_peak_switch == "off" :
    if not options.broad_peak_file_filter_sort and  not options.output_name or not options.output_path or not options.input_bam_file or not options.chrom_size_file :
        print("Missing parameter, please read the parameter description")
        parser.print_help()
        exit() 
elif options.wellington_footprints_switch == "off" and options.macs2_call_peak_switch == "on" :
    if not options.footprint_file and  not options.output_name or not options.output_path or  not options.input_file :
        print("Missing parameter, please read the parameter description")
        parser.print_help()
        exit()
elif options.wellington_footprints_switch == "off" and options.macs2_call_peak_switch == "off" :
    if not options.footprint_file and  not options.broad_peak_file_filter_sort or not options.output_path or  not options.output_name :
        print("Missing parameter, please read the parameter description")
        parser.print_help()
        exit()
...
broad_peak_file_filter=broad_peak_file+'_pfilter'
if options.macs2_call_peak_switch== "on" :
    ...
    subprocess.call( "rm " + broad_peak_file_filter  + " " + options.output_path+'/'+options.output_name+ "_control_lambda.bdg  "+ options.output_path+'/'+options.output_name+ "_peaks.gappedPeak  ",shell=True)
...
if options.wellington_footprints_switch=="on" :
    ...
    subprocess.call("bedtools merge -i  "+ footprint_path+"WellingtonFootprints_overlap_score_sort.bed -d -10 -c 4,10  -o distinct,min  | awk '{print $1 "+ '"\t" $2'+ ' "\t" $3 '+' "\t" $4  "\t" '+ " $5*-1}' > " + footprint_path+"WellingtonFootprints_score.bed" ,shell=True)
    footprint_score_file=footprint_path+"WellingtonFootprints_score.bed"
    subprocess.call( "rm " + footprint_path+"WellingtonFootprints_overlap_score_sort.bed  "  + footprint_path+"WellingtonFootprints_overlap_score.bed  "+ footprint_out_path+"/"+options.output_name+".WellingtonFootprints_tmp.wig  "  ,shell=True)
################cut_wig_TO_bw
...
def identify_super_peak (ATAC_peak,footprint,cut_off,out_path) :
    ...
    peak_overlap_footprint=open(out_path+"/"+options.output_name+"_broadPeak_overlap_"+score+"_footprints.bed","w")
    ...
    subprocess.call("awk '{print $1"+ '"-" $2'+ '"-"'+ "$3}' " +out_path+"/"+options.output_name+"_broadPeak_overlap_"+score+"_footprints.bed"+ " | sort |uniq -c  |awk '{print $2 "+'"\t"'+" $1}' > " +out_path+"/"+options.output_name+"_broadPeak_footprint_number",shell=True)
