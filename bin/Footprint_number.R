#!/usr/bin/Rscript
Args <- commandArgs()
footprint_number=read.table(Args[6],header=F,quote='\t',row.names = 1) 
data<-data.frame(number=sort(c(footprint_number[,1])),sort=1:length(footprint_number[,1]),group=c(rep("Non",round(length(footprint_number[,1])*0.8)),rep("Yes",length(footprint_number[,1])-round(length(footprint_number[,1])*0.8))))
...
labs(y="Footprint Number",x="ATAC Peak")+scale_color_manual(values =c("grey","#FF6600")) +scale_fill_manual(values =c("grey","#FF6600"))+
...
geom_text(data=text_data,aes(x=text_data[,1],y=text_data[,2],label=text_data[,3]),size=2.5,col="#FF6600")
dev.off()
cut_off<-NULL
for (i in seq(1,30,1)) {
...
colnames(cut_off)<-c("Percentage","footprint_number")
