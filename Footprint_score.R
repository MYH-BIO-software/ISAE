...
Args <- commandArgs()
pvalue=read.table(Args[6],header=T,quote='\t') 
data<-data.frame(pvlaue=sort(c(-pvalue[,4])),sort=1:length(pvalue[,4]),group=c(rep("Non",round(length(pvalue[,4])*0.8)),rep("Yes",length(pvalue[,4])-round(length(pvalue[,1])*0.8))))
...
labs(y="-log10(Footprint Pvalue)",x="Footprint")+scale_color_manual(values =c("grey","#9400D3")) +scale_fill_manual(values =c("grey","#9400D3"))+
...
colnames(cut_off)<-c("Percentage","Pvalue")
